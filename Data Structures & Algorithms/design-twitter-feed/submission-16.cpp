...
public:
    ...
    void postTweet(int userId, int tweetId) {
        ...
        if (tweetMap[userId].size() > 10) {
            tweetMap[userId].earse(tweetMap[userId].begin());
        }
        count--;
    }
    ...
    vector<int> getNewsFeed(int userId) {
        ...
        for (int followeeId : followMap[userId]) {
            if (tweetMap.count(followeeId)) {
                int index = (int)tweetMap[userId].size() - 1;
        ...
        while (!minHeap.empty() && res.size() < 10) {
            ...
            if (index >= 0) {
               auto [count, tweetId] = tweetId[followeeId][index];
               minHeap.push({count, tweetId, followeeId, index - 1});
            }
        }
        return res;
    ...
    void unfollow(int followerId, int followeeId) {
        if (followMap[followerId].count(followeeId)) {
            followMap[followerId].earse(followeeId)
        }
    }
};
